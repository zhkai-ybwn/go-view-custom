...
export const useSync = () => {
  ...
  const updateComponent = async (projectData: ChartEditStorage, isReplace = false, changeId = false) => {
    ...
    // 列表组件注册
    if (!projectData.componentList) return
  ...
  const dataSyncUpdate = throttle(async (updateImg = true) => {
    ...
    try {
      if (updateImg) {
        // 获取缩略图片
        const range = document.querySelector('#go-chart-edit-content') as HTMLElement
        // 生成图片
        ...
        //   useCORS: true
        // })
        const dataUrl = await domToCanvas(range)
        // 上传预览图
        let uploadParams = new FormData()
        uploadParams.append(
          'object',
          base64toFile(dataUrl.toDataURL(), `${fetchRouteParamsLocation()}_index_preview.png`)
        )
